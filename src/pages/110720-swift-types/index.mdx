...
With so many types like `class`, `struct` and `enum`, what are the differences and similarities?

Properties are either stored, or computed. Store properties have fixed values. Computed properties are calculated every time they are referenced. There are also simple observers on properties that listen to changes via callbacks. These observers are `willSet` and `didSet`.
...
###Computed Propert — Getters & Setters
